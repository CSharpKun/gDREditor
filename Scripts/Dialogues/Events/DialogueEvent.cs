...
namespace DREditor.Dialogues.Events
{
...
#if UNITY_EDITOR
		void EditorUI(object value = null);
		void ToggleHelpBox();
		void ShowHelpBox();
#endif
    }
    ...
    static public class DialogueEventSystem
    {
        ...
        public static void TriggerEvent(string eventName)
        {
            if (_eventDict.TryGetValue(eventName, out DiaEvent thisEvent))
            {
                UnityEngine.Debug.Log("DREditor <color=blue>(DialogueEventSystem)</color>: Triggering " + eventName + " event.");
                thisEvent.Invoke(null);
            }
            else
            {
                UnityEngine.Debug.LogWarning($"DREditor (DialogueEventSystem): Tryna trigger {eventName}, but it somehow doesn't exist.\nYou need to have to use StartListening first before you invoke the event. This is ignorable if it's intended.");
            }
        }
