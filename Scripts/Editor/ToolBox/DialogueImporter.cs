...
namespace DREditor.Toolbox
{
    ...
    public class DialogueImporter : ScriptableWizard
    {
        ...
        public NameMatchOption nameMatch = NameMatchOption.FirstNameOnly;
        ...
        public int maxLinesPerDialogue = 50;
        [Tooltip("The maximimum amount of text characters allowed in a line")]
        public int maxCharactersPerLine = 200;
        [Tooltip("Enable this if you want a new dialogue if a certain number of empty lines are found")]
        public bool newOnEmptyLine = false;
        [Tooltip("The number of lines required to separate to a new dialogue asset")]
        public int emptyLineLimit = 2;
        [Tooltip("Custom Regex string")]
        public string regexString = @"^(.+?)(?: *)/(?: *)(.+?)(?: *):(?: *)(.*)$";
        [Tooltip("Capturing groups")]
        public List<DialogueKeywords> keyList = new List<DialogueKeywords>() { DialogueKeywords.Name, DialogueKeywords.Expression, DialogueKeywords.DialogueText };
                ...
                if (path.Length != 0)
                {
                    ...
                    using (StreamReader reader = new StreamReader(path, System.Text.Encoding.UTF8))
                    {
                        ...
                        while (!reader.EndOfStream)
                        {
                            ...
                            {
                                if ((newOnEmptyLine && emptyLineCounter >= emptyLineLimit) 
                                    || currentDialogue.Lines.Count + 1 > maxLinesPerDialogue)
                                {
                                    /* - Note from Sweden
                                     * So there will be instances where if there are 2 empty lines in the 
                                     * text document, it will try to create a new dialogue, 
                                     * I've Decided to make it an option in the wizard
                                     */
                                ...
                                if (regex.IsMatch(line))
                                {
                                    ...
                                    //Debug.Log(normalizedLines.Count);
                                    currentDialogue.Lines.AddRange(normalizedLines);
                                } else 
                                {
                                    if(lastLine == null)
                                    {
                                        EditorUtility.DisplayDialog("Format incorrect", "The file didn't comply with the format\n" +
                                            "on line " + currentLine, "OK");
                                        return;
                                    }
                                    ...
                                    lineInfo.aliasIndex = lastLine.AliasNumber;
                                ...
                                if(lineInfo.expression != null)
                                {
                                    ...
                                    try
                                    {
                                        //Debug.Log("CurrentLine at Expression: " + currentLine);
                                        //Debug.Log("CurrentLine with Expression: " + lineInfo.expression.Name);
                                        //Debug.Log("CurrentDialogue Count at Expression: " + currentDialogue.Lines.Count);
                                        //Debug.Log(dialoguesToSave.Count);
                                    ...
                                    {
                                        Debug.LogWarning("WARNING! Importer had trouble importing expression. " +
                                            "This most commonly occurs when you're using the make new at empty option." +
                                            "\n If the imported dialogue appears to be correct, then you can ignore this warning.");
                                        currentLine = 0;
                                        currentDialogue.Lines[currentLine].Expression = lineInfo.expression;
                                        currentDialogue.Lines[currentLine].ExpressionNumber = lineInfo.expressionNumber;
                                    }
                                ...
                                //Debug.Log("CurrentLine at end is: " + currentLine);
                                //Debug.Log("CurrentDialogue Count at end is: " + currentDialogue.Lines.Count);
                                currentLine++;
                                emptyLineCounter = 0;
                            }
        ...
        private List<Line> BreakLine(DialogueLineInfo lineInfo)
        {
            ...
            int aliasNumber = lineInfo.aliasIndex;
            ...
            if(lineContent.Length + lineLenghtOffset <= maxCharactersPerLine)
            {
                ...
                line.AliasNumber = aliasNumber+1;
                line.Text = "";
                if(speakerName != null)
                {
        ...
        private Character FindCharacterInDB(string name, out int index, out int alias)
        {
            index = -1;
            alias = -1;
            ...
            for(int i=0;i<database.Characters.Count;i++)
            {
                ...
                if (name.Contains(stu.FirstName))
                {
                    found = database.Characters[i];
                    index = i;
                    if(stu.Aliases.Count > 0)
                    {
                        for (int j = 0; j < stu.Aliases.Count; j++)
                            if (name.Contains(stu.Aliases[j].Name))
                                alias = j;
                    }
                }
                switch (nameMatch)
        ...
        private AudioClip FindVoice(string voiceName) => Resources.Load<AudioClip>("Voicelines/" + name + " Voicelines/" + voiceName);
        private AudioClip FindSoundFX(string soundFXName) => Resources.Load<AudioClip>("SFX/" + soundFXName);
        private int GetCharacterDatabase()
        {
            ...
            if (_databaseguids.Length == 1)
            {
                ...
                database = AssetDatabase.LoadAssetAtPath<CharacterDatabase>(_databasepath);
                return 1;
            }
