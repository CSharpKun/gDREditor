...
namespace DREditor.Toolbox
{
    ...
    public class TrialDialogueImporter : ScriptableWizard
    {

        public enum NameMatchOption
        {
            FirstNameOnly, FullName, LastNameOnly
        }
        ...
        [MenuItem("Tools/DREditor/Import Trial Dialogues")]
        public static void CreateWizard()
        {
            DisplayWizard<TrialDialogueImporter>("Import Dialogues", "Import");
        }
        ...
        private void OnWizardCreate()
        {
            ...
            if (database)
            {
                ...
                List<TrialDialogue> dialoguesToSave = new List<TrialDialogue>();
                float progress = 0;
                ...
                if (path.Length != 0)
                {
                    ...
                    using (StreamReader reader = new StreamReader(path, System.Text.Encoding.UTF8))
                    {
                        ...
                        TrialDialogue currentDialogue = CreateInstance<TrialDialogue>();
                        currentDialogue.Speakers = database;
                        currentDialogue.Lines = new List<TrialLine>();
                        ...
                        while (!reader.EndOfStream)
                        {
                            ...
                            else
                            {
                                    ...
                                    || currentDialogue.Lines.Count + 1 > maxLinesPerDialogue)
                                {
                                     ...
                                     */
                                    currentDialogue = CreateInstance<TrialDialogue>();
                                    currentDialogue.Speakers = database;
                                    currentDialogue.Lines = new List<TrialLine>();
                                    dialoguesToSave.Add(currentDialogue);
                                    lastLine = null;
                                ...
                                if (regex.IsMatch(line))
                                {
                                    ...
                                    for (int i = 1; i < m.Groups.Count; i++)
                                    {
                                        ...
                                        switch (keyword)
                                        {
                                            case DialogueKeywords.Name:
                                                ...
                                                for (int x = 0; x < lineInfo.character.Aliases.Count; x++)
                                                {
                                                    if (group.Value == lineInfo.character.Aliases[x].Name)
                                                    {
                                                        lineInfo.aliasNum = x;
                                                    }
                                                }
                                    ...
                                    List<TrialLine> normalizedLines = BreakLine(lineInfo);
                                ...
                                if(lineInfo.aliasNum != -1)
                                {
                                    currentDialogue.Lines[currentLine].AliasNumber = lineInfo.aliasNum+1;
                                }
                ...
                {
                    TrialDialogue dia = dialoguesToSave[i];
        ...
        private List<TrialLine> BreakLine(DialogueLineInfo lineInfo)
        {
            ...
            List<TrialLine> result = new List<TrialLine>();
            ...
            else
            {
                ...
                TrialLine currentLine = new TrialLine();
        ...
        private Character FindCharacterInDB(string name, out int index)
        {
            ...
            for (int i = 0; i < database.Characters.Count; i++)
            {
                ...
                for(int x = 0; x < stu.Aliases.Count; x++)
                {
                    Alias alias = stu.Aliases[x];
                    if(alias.Name == name)
                    {
                        found = database.Characters[i];
                        index = i;
